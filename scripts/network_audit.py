#!/usr/bin/env python3
"""
Unified HPC Network & Node Diagnostic Tool (v5.4 - Interface Filtering Edition)
- Adds `--no-eth-check` flag to ignore specific interfaces or prefixes (e.g. eth0 em2 eno)
- Python 3.6+ backwards compatible subprocess execution (fixed capture_output)
- Bulk diagnostic execution via `pdsh -R ssh` with Base64 payload encoding
...
class HPCNetworkAuditor:
    def __init__(self, args):
        ...
        self.ignored_ifaces = args.no_eth_check or []
        self.results = {
    ...
    def run_cmd(self, cmd, timeout=60):
        """Executes local system commands with legacy Python 3.6 compatibility."""
        try:
            res = subprocess.run(
                cmd,
                shell=True,
    ...
    def get_iface_ignore_pattern(self):
        """Constructs a regex pattern for grep -v -E to filter out ignored interfaces."""
        base_ignore = ['lo', 'veth', 'virbr']
        combined = base_ignore + self.ignored_ifaces
        return "|".join(combined)

    def expand_hostlist(self, raw_nodes):
        """Expands Slurm-style node ranges like node00[01-70] into individual hostnames."""
        expanded_nodes = []
        has_scontrol = shutil.which("scontrol") is not None
    ...
    def run_pdsh_script(self, raw_script, timeout=60):
        """Encodes bash script to Base64 and executes across nodes via pdsh -R ssh."""
        if not self.pdsh_target_str:
            return {}

        b64_payload = base64.b64encode(raw_script.strip().encode('utf-8')).decode('utf-8')
        cmd = f"pdsh -R ssh -f 64 -u 15 -w '{self.pdsh_target_str}' 'echo {b64_payload} | base64 -d | bash'"
        ...
        if lines_parsed == 0 and code != 0:
            print(f"    [!] Warning: pdsh returned exit code {code}. Sample output: {out[:120]}")
    ...
    def auto_detect_environment(self):
        ...
        pdsh_path = shutil.which("pdsh")
        if not pdsh_path:
            print("[!] CRITICAL: 'pdsh' binary not found in system PATH by Python.")
            sys.exit(1)
        ...
        if self.ignored_ifaces:
            print(f"    - Ignored Ethernet Devices/Prefixes: {', '.join(self.ignored_ifaces)}")

    # -------------------------------------------------------------------------
    # PHASE 2: Base64 Encoded Bulk pdsh Sweeps
    # -------------------------------------------------------------------------
    def audit_fabric_and_nodes(self):
        ...
        if self.results["fabric_type"] == "InfiniBand" and shutil.which("ibqueryerrors"):
        ...
        ignore_pattern = self.get_iface_ignore_pattern()

        # SWEEP 1: PCIe Speed Verification
        print("    - [Sweep 1/4] Auditing PCIe bus negotiation...")
        pcie_script = "lspci -vvv"
        pcie_data = self.run_pdsh_script(pcie_script)
        ...
        ip -s link
        echo "===BONDING==="
        cat /proc/net/bonding/bond* 2>/dev/null
        """
        db_data = self.run_pdsh_script(drop_bond_script)
        for node, lines in db_data.items():
            full_text = "\n".join(lines)
        ...
        # SWEEP 3: Interface Link State & Transceiver Optical Power (Filtered)
        print("    - [Sweep 3/4] Auditing Link states & optical transceiver power levels...")
        opt_script = f"""
        for i in $(ip -o link | awk -F': ' '{{print $2}}' | grep -v -E '{ignore_pattern}'); do
            echo "===IFACE:$i==="
            ethtool "$i" 2>&1
            ethtool -m "$i" 2>&1
        done
        """
        opt_data = self.run_pdsh_script(opt_script, timeout=90)
        ...
        # SWEEP 4: Optional Hardware Self-Tests (Filtered)
        if self.args.run_hw_tests:
            ...
            """
            test_data = self.run_pdsh_script(test_script, timeout=120)
    ...
    def run_benchmarks(self):
        ...
        for node_a, node_b in node_pairs:
            ...
            self.run_cmd(f"pdsh -R ssh -w '{node_a}' 'pkill -9 ib_write_bw 2>/dev/null'")
            self.run_cmd(f"pdsh -R ssh -w '{node_a}' 'ib_write_bw -d mlx5_0 -i 1 --report_gbits > /tmp/bw_srv.log 2>&1 &'")
            time.sleep(1)
            
            client_out, code = self.run_cmd(f"pdsh -R ssh -w '{node_b}' 'ib_write_bw -d mlx5_0 -i 1 --report_gbits {node_a}'")
            ...
            lat_out, l_code = self.run_cmd(f"pdsh -R ssh -w '{node_b}' 'ib_read_lat -d mlx5_0 -i 1 {node_a}'")
            if l_code == 0:
...
# -------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Unified HPC Network Auditor (pdsh Base64 Edition)")
    parser.add_argument("--nodes", nargs="+", help="Node list or range expressions (e.g. hpctpa3pc[0001-0070] hpctpa3pl[0001-0003])")
    parser.add_argument("--no-eth-check", nargs="+", default=[], help="List of Ethernet device names or prefixes to ignore (e.g. eth0 em2 p3p2 eno)")
    parser.add_argument("--run-hw-tests", action="store_true", help="Execute ethtool --test hardware self-diagnostics across non-ignored interfaces")
