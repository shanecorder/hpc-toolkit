#!/usr/bin/env python3
"""
Unified HPC Network & Node Diagnostic Tool (v5.1 - Admin / pdsh Edition)
- Bulk diagnostic execution via `pdsh` (bypasses Slurm job queues completely)
- Uses Python native `shutil.which` for reliable binary detection
...
class HPCNetworkAuditor:
    ...
    def auto_detect_environment(self):
        ...
        # Check pdsh binary availability using native shutil
        pdsh_path = shutil.which("pdsh")
        if not pdsh_path:
            print("[!] CRITICAL: 'pdsh' binary not found in system PATH by Python.")
            print("    Please ensure /usr/bin is in PATH or pdsh is installed.")
            sys.exit(1)
